...
test("projects route presents four editorial project cards with project links", async () => {
  ...
  assert.match(html, /Northwestern Dining Ranked/);
  ...
  assert.match(projectsSource, /className="project-demo-note">Demo coming soon<\/p>/);
  assert.equal(projectsSource.match(/Demo coming soon/g)?.length, 1);
  const homeMemoryIndex = projectsSource.indexOf('slug: "homememory"');
